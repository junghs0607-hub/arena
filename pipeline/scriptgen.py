...
PLACEHOLDERS = ("topic", "scene_count", "duration", "tone")
...
def build_prompt(template: str, *, topic: str, scenes: int, duration: int, tone: str) -> str:
    values = {
        "topic": topic,
        "scene_count": scenes,
        "duration": duration,
...
def _mock_answer(prompt: str) -> str:
    """오프라인 시연/테스트용: 프롬프트에서 주제를 뽑아 형식만 맞춘 대본."""
    m = re.search(r"주제:\s*(.+)", prompt)
    topic = (m.group(1).strip() if m else "이 주제")[:30]
    sc = re.search(r"씬\s*(\d+)개", prompt)
    ...
    body = [hook] + middles[: max(0, scenes - 2)] + [outro]
    return "\n\n".join(body[:scenes])
